...
def block_diagonal_mask(seq_lens):
    """
    Create a block diagonal attention mask for transformer models where attention is
    allowed only within the same sequence block in the batch.

    Args:
        seq_lens (list or torch.Tensor): A list or tensor of integers representing the 
                                         lengths of each sequence in a batch.

    Returns:
        torch.Tensor: A mask tensor with shape (batch_size, max_seq_len, max_seq_len)
                      where 0 indicates positions that can be attended to and -inf indicates 
                      masked positions.
    """
    batch_size = len(seq_lens)
    max_seq_len = max(seq_lens)
    mask = torch.full((batch_size, max_seq_len, max_seq_len), float('-inf'))

    for idx, length in enumerate(seq_lens):
        mask[idx, :length, :length] = 0
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        attn_bias = None
        if mask is not None:
            attn_bias = block_diagonal_mask([N] * B, mask)

        x =  efficient_dot_product_attention_pt(q, k, v, mask, attn_bias)
